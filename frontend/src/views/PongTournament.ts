// TRANSCENDENCE/frontend/src/views/PongTournament.ts
...
export function renderPongTournament(container: HTMLElement) {
	...
	function update() {
		...
		if (player2.isAlive) {
			if (gameMode === 'ONE_PLAYER' || localStorage.getItem('opponentUsername')?.includes('IA')) {
	...
	async function startGame() {
		...
		const tournamentMatchId = localStorage.getItem('tournamentMatchId');

		if (tournamentMatchId) {
			matchId = parseInt(tournamentMatchId, 10);
			match_type = 'tournament';
			console.log(`Iniciando partida de torneo con ID de partida: ${matchId}`);
	...
	async function endGame(winner: number) {
		...
		if ((gameMode === 'ONE_PLAYER' && winner === 2) || (localStorage.getItem('opponentUsername')?.includes("IA") && winner === 2)) {
		...
		if (matchId) {
			...
			try {
				...
				if (localStorage.getItem('tournamentMatchId')) {
					localStorage.removeItem('tournamentMatchId');
